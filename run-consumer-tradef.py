...
PATHS = {
    "re-local-remote": {
        ...
        "path-to-output-dir": "D:/monica_saxony/out/out/",
        "path-to-csv-output-dir": "D:/monica_saxony/out/csv-out/"
    },
    "mbm-local-remote": {
        "path-to-data-dir": "data/",
...
def create_output(msg):
    ...
    monthly_data = defaultdict(lambda: defaultdict(lambda: {"TraDef": {}}))
    last_crop_by_cmc = {}
    saw_monthly = False
    ...
    for data in msg.get("data", []):
        ...
        if orig_spec == '"monthly"':
            saw_monthly = True
            for vals in results:
                cmc = vals.get("CM-count")
                year = vals.get("Year")
                month = vals.get("Month")
                if cmc is None or year is None or month is None:
                    continue
                crop = (vals.get("Crop") or "").strip()

                tra = vals.get("TraDef", None)
                if crop and tra is not None:
                    monthly_data[(cmc, year)]["TraDef"][int(month)] = float(tra)

                if crop:
                    last_crop_by_cmc[cmc] = crop
                    cm_count_to_vals[cmc]["Crop"] = crop
            ...
            for vals in results:
                if "CM-count" in vals:
                    cmc = vals["CM-count"]
                    cm_count_to_vals[cmc].update(vals)
                    crop = vals.get("Crop")
                    if crop:
                        last_crop_by_cmc[cmc] = crop
...
def write_row_to_grids(row_col_data, row, ncols, header, path_to_output_dir, path_to_csv_output_dir, setup_id):
    ...
    for m in range(1, 13):
        output_grids[f"TraDef_monthly_{m}"] = {"data": make_dict_nparr(), "cast-to": "float", "digits": 5}
    ...
    for key, y2d_ in output_grids.items():
        ...
        for (cm_count, year), row_arr in y2d.items():
            crop = cmc_to_crop.get(cm_count) or "none"
            crop = crop.replace("/", "").replace(" ", "")
            if not crop:
                crop = "none"
            month_tag = ""
            if key.startswith("TraDef_monthly_"):
                try:
                    m = int(key.split("_")[-1])
                    month_tag = f"{m:02d}"
                    key2 = "TraDef"
            ...
            path_to_file = os.path.join(
                path_to_output_dir,
                f"{cm_count}_{crop}_{key2}_{year}_{month_tag}.asc"
            )
