...
TEMPLATE_PATH_LATLON = "data/climate/{region_name}/latlon-to-id-{region_name}-{rcm}.json"
...
def get_vg_climate_id(region_name, lat, lon, paths, gcm, rcm):
    ...
    for entry in latlon_to_id:
        coord, climate_id_list = entry
        lat2, lon2 = coord
        ...
        if dist < min_dist:
            min_dist = dist
            nearest_id = climate_id

    return str(nearest_id)
...
def run_producer(server={"server": None, "port": None}, shared_id=None):
    ...
    for _, setup_id in enumerate(run_setups):
        ...
        if region_name and len(region_name) > 0:
            ...
            path_to_soil_grid = paths["path-to-data-dir"] + DATA_GRID_SOIL
            mask = create_mask_from_shapefile(VG_REGIONS, region_name, path_to_soil_grid)
        ...
        for srow in range(0, srows):
            ...
            for scol in range(0, scols):
                ...
                # vg_climate_id = get_vg_climate_id(region_name, slat, slon, paths)
                vg_climate_id = get_vg_climate_id(region_name, slat, slon, paths, gcm, rcm)
                ...
                subpath_to_csv = TEMPLATE_PATH_CLIMATE_CSV.format(gcm=gcm, rcm=rcm, scenario=scenario, ensmem=ensmem,
                                                                  version=version, region_name=region_name,
                                                                  vg_climate_id=vg_climate_id)
                for _ in range(4):
